...
# имеетоплива в час при ра
# #   скорость погрузки, расход тботе
# может
...
class Warehouse:
    ...
    def truck_arrived(self, truck):
        ...
        truck.place = self
        print('На склад {} прибыл грузовик {}'.format(self.name, truck))
    ...
    def truck_ready(self, truck):
        ...
        print('Склад {}, грузовик {} готов '.format(self.name, truck))
...
class AutoLoader(Vehicle):
    ...
    def load(self):
        ...
        elif truck_cargo_rest < self.bucket_capacity:
...
loader_1 = AutoLoader(model='Bobcat', bucket_capacity=1020, warehouse=moscow, role='loader')
...
trucks = []
for i in range(5):
    truck = Truck(model='Man #{}'.format(i), body_space=5000)
    moscow.truck_arrived(truck)
    trucks.append(truck)
for i in range(5):
    truck = Truck(model='Volva #{}'.format(i), body_space=6000)
    moscow.truck_arrived(truck)
    trucks.append(truck)
